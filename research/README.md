...
- **Experiment 9: Model Robustness** — All 8 experiments used GBM with stochastic IV. Run the Exp 6 optimal configs across all 4 price models (GBM, Heston, Jump, Heston-Jump) at vol levels 40–80%. Exp 8 proved the regime filter exploits IV/RV *variance*, not just mean VRP — but GBM's OU process produces smooth, normally-distributed IV/RV ratios. Heston's vol clustering could cause RF to trigger in bursts (skip 50 consecutive cycles, then fire 30 in a row), producing path-dependent outcomes. Jump processes may cause sudden assignment cascades the δ0.20 strategies can't absorb. Exp 7 showed Active is drift-immune and Exp 8 confirmed VRP-resilience — but both were GBM-only. If these properties break under Heston-Jump, the "all-weather" claim needs qualification. This is the single largest remaining blind spot.
- **Experiment 10: Multi-Year Horizon** — All experiments use 365-day simulations. Run the Exp 6 optimal configs over 2-year and 5-year horizons at drift levels 0%, +5%, -30% and VRP levels 10%, 15%. Exp 7 showed Active earns 18% APR at -30% drift over 1 year — does that survive 2+ years of sustained decline, or does compounding drawdown eventually overwhelm premium income? Exp 8 showed the regime filter skips ~94–97% of cycles — over multiple years, the ~3–6% acceptance rate compounds differently than 1-year projections suggest. Also tests whether `skipBelowRatio` thresholds remain stable as vol regimes shift over longer horizons.
- **Experiment 11: Lookback × Cycle Interaction** — IV/RV lookback is fixed at 20 days across all 8 experiments. Exp 8 confirmed RF exploits IV/RV variance, not mean VRP — this makes lookback window quality the primary tuning lever for the single most important feature. Active's 3-day cycle with 20-day lookback may act on stale regime signals; Conservative's 30-day cycle might benefit from longer lookback. Test lookback values 5–60 days crossed with all three cycle lengths at drift levels 0% and -30%, VRP levels 10% and 15%. Since RF is the only feature that matters for Moderate/Active, optimizing its input signal has outsized impact.

### Medium — Parameter refinement within proven framework

- **Experiment 12: Adaptive Calls Rehabilitation** — Exp 6 found adaptive calls harmful for Moderate/Active (up to -0.383 ΔSharpe) due to a -0.231 interaction with regime filter. But AC helps Conservative (+0.118). The conflict may be caused by AC's wide delta band (0.10–0.50) and aggressive skipping at higher base deltas. Test narrower bands (0.15–0.30), higher `skipThresholdPct` values (0.5–2%), and `minStrikeAtCost=false` for Moderate/Active to see if a tuned AC can coexist with RF. If solvable, all three strategies benefit from adaptive calls.
- **Experiment 13: Deployment Signal Estimation** — *(Expanded from "Drift Estimation Signal".)* Exp 7 established drift floors (-15% Conservative, -10% Moderate) and Exp 8 established VRP floor (10%). In practice, both must be estimated from trailing market data. Test: (a) trailing windows (30d, 60d, 90d annualized return) as drift estimators, (b) trailing IV-RV spread (20d, 40d, 60d) as VRP estimators, (c) a combined deployment signal that switches between Active (any drift, VRP≥5%), Moderate (drift>-10%, VRP≥10%), and Conservative (drift>-15%, VRP≥10%). Key questions: does estimation lag cause whipsaw at boundaries, or does the smooth Sharpe degradation (Exp 7 finding #6, Exp 8 finding #4) provide enough margin? Does a single "deploy/pause" signal outperform per-strategy switching?

### Low — Speculative or requires engine changes

- **Experiment 14: Vol-Adaptive Skip Threshold** — *(Demoted from Medium.)* Experiments 4–6 confirmed optimal `skipBelowRatio` varies by strategy (Conservative: 1.0, Moderate/Active: 1.2). Exp 7 showed skip rates stable across drift (<2% variation). Exp 8 showed thresholds don't need recalibration across VRP levels either. Test a dynamic formula `skipBelowRatio = base + k × annualizedVol` to unify per-strategy tuning. Demoted because fixed thresholds work across drift (Exp 7), VRP (Exp 8), and vol regimes — the main benefit is configuration simplification rather than performance improvement.
