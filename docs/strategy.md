...
IDLE_CASH ───SELL_PUT───→ SHORT_PUT ←─── roll ───┐
                              │                   │
                   expired OTM│assigned            │ (OTM + DTE ≤ threshold)
                   ┌──────────┘──────────┐         │
                   ↓                     ↓         │
...
| `OPTION_ROLLED` | ROLL signal executed: old option closed, new option opened. Carries `optionType: "put" \| "call"` identifying which leg was rolled. |
...
| `BasePutRule` | 100 | `idle_cash` | `SELL_PUT` | Sell OTM put at `targetDelta`. DTE uses `rollPut.initialDTE` when configured, else `cycleLengthDays`. |
