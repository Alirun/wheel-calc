...
| `SKIP` | `LowPremiumSkipRule`, `StopLossCooldownRule` | rule, reason |
...
Priority ordering: risk rules (1–2) preempt skip rules (50) which preempt selection rules (100). `StopLossRule` and `RollCallRule` fire mid-cycle via dedicated triggers in `simulate.ts`; `RollCallRule` requires `!stopLossTrigger` to not compete with an active stop-loss.
...
1. **Decision point**: No open option, open option has expired (`day ≥ expiryDay`), roll trigger fires mid-cycle, or stop-loss trigger fires mid-cycle.
